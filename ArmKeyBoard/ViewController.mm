...
-(void) doContourOperationTarget: (cv::Mat &)targtImg  Src:(cv::Mat &)srcImg Mix:(cv::Mat &)mixImg{
    ...
    for( int i = 0; i< mycontours.size(); i++ ) {
        ...
        drawContours( contourdrawing, mycontours, i, color, 1, 8, hierarchy, 0, cv::Point() );
    }
    ...
    for( size_t i = 0; i < mycontours.size(); i++ ) {
        convexHull( cv::Mat(mycontours[i]), hull[i], false );
    ...
    {
        cv::Scalar color = cv::Scalar( rng.uniform(0, 255), rng.uniform(0,255), rng.uniform(0,255) );
        drawContours( drawing, hull, (int)i, color, 2, 8, vector<cv::Vec4i>(), 0, cv::Point() );
    }
    ...
    //_mainImage.image = [self UIImageFromCVMat:mixImg];
